...
import { Block, BlockQuery, BlockTransactionQuery, Channel, DatabaseNames, State, Transaction, TransactionQuery } from '../../types';
...
export class CouchDatabase implements DatabaseAdapter {
	...
	constructor(dbConfig: string | Nano.Configuration) {
		...
		this.channels = this.nano.db.use<Channel>(DatabaseNames.CHANNELS.toLowerCase());
		this.blocks = this.nano.db.use<Block>(DatabaseNames.BLOCKS.toLowerCase());
		this.transactions = this.nano.db.use<Transaction>(DatabaseNames.TRANSACTIONS.toLowerCase());
	...
	public async setup(channels: string[]): Promise<void> {
		...
		for (const channelName of channels) {
			await this.channels.insert({ name: channelName, height: 0, lastHash: '' }, channelName).catch(() => { });
		}
	}
	...
	public async getBlocks({ sort, page, direction, size, query }: BlockQuery): Promise<Block[]> {
	...
	public async getBlockTransactions({ id, sort, page, direction, size }: BlockTransactionQuery): Promise<Transaction[]> {
	...
	public async getTransactions({ sort, page, direction, size, query }: TransactionQuery): Promise<Transaction[]> {
		const { docs } = await this.transactions.find({
			skip: size * (page - 1),
			limit: size,
			selector: query,
