import { Block as FabricBlock, BlockData } from 'fabric-client';
import { Gateway, GatewayOptions } from 'fabric-network';
import * as _ from 'lodash';
import { DatabaseAdapter } from './adapters';
import { Channel, ChannelOption, NetworkConfig } from './types';
...
export class ExplorerSync {
  private readonly logger = getLogger('Sync');

  private readonly networkConfig: string | NetworkConfig;
  ...
  public async start(): Promise<void> {
    await this.gateway
      ...
      .catch((error) =>
        abort('[Explorer.Sync] Unable to connect to gateway', error)
      );

    await this.database
      .connect()
      .catch((error) =>
        abort('[Explorer.Sync] Database connection problem', error)
      );

    await this.database
      .setup(this.channels.map((c) => c.name))
      .catch((error) =>
        abort('[Explorer.Sync] Unable to run setup on database', error)
      );

    this.logger.debug(`Write interval set to ${this.writeInterval}`);
    this.timer = setInterval(
      async () => await this.flushPendingWrites(),
      this.writeInterval
    );
    ...
    for (const channel of this.channels) {
      const { name, options } = channel;
      let { startBlock = 'auto' } = options;
      if (startBlock === 'auto') {
      ...
      await this.addBlockListener(name, startBlock).catch((error) =>
        abort(
          `[Explorer.Sync] Unable to add block listener: name=${name}, startBlock=${startBlock}`,
          error
        )
      );
    }
  ...
  private async addBlockListener(channelName: string, startBlock) {
    const channel = await this.gateway
      ...
      .catch((error) =>
        abort(`[Explorer.Sync] Unable to get channel: ${channelName}`, error)
      );

    this.listeners.push(
      await channel.addBlockListener(
        'listener',
        async (error: Error, event: FabricBlock) =>
          await this.onEvent(error, event, channelName),
        { startBlock }
      )
    );
  ...
  private async onEvent(error: Error, event: FabricBlock, channelName) {
    if (error) {
  ...
  private async onBlock(event: FabricBlock, channelName: string) {
    const hash = event.header.data_hash.toString('hex');
    const shortId = hash.substr(0, 16);
    const height = parseInt(event.header.number, 10);
    const transactions = event.data.data;
    const timestamp = transactions[0].payload.header.channel_header.timestamp;
    const previousHash = event.header.previous_hash.toString('hex');

    if (height <= (await this.getChannelHeight(channelName))) {
      this.logger.debug(
        `[${channelName}/${height}] Skipped Block: ${shortId}'...'`
      );
      return;
    }
  ...
  private async onTransaction(
    ...
    const channelHeader = _.get(
      transactionData,
      'payload.header.channel_header'
    );
    const chaincodeResponse = _.get(
      transactionData,
    ...
    if (chaincodeResponse) {
      for (const item of chaincodeResponse.results.ns_rwset) {
        ...
        for (const write of item.rwset.writes) {
          chaincodeWrites[write.key] = write.isDelete
            ? null
            : this.tryJsonParse(write.value);
        }
      }
    }
    ...
    const transaction = {
      ...
      chaincodeName: chaincodeResponse
        ? chaincodeResponse.chaincode_id.name
        : null,
      chaincodeVersion: chaincodeResponse
        ? chaincodeResponse.chaincode_id.version
        : null,
      chaincodeResponseStatus: chaincodeResponse
        ? chaincodeResponse.response.status
        : null,
      chaincodeResponse: chaincodeResponse
        ? this.tryJsonParse(chaincodeResponse.response.payload)
        : null,
    ...
    this.logger.info(
      `[${transaction.channelName}/${
        block.height
      }] New Transaction: ${transaction.id.substr(0, 16)}'...'`
    );
  ...
  private async getChannelHeight(name: string): Promise<number> {
    if (!this.writeCache[name]) {
      ...
      this.logger.info(
        `[${name}] Loaded height from database: ${channel.height}`
      );
    }
